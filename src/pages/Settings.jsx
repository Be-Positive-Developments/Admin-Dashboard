  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">{t('settings_desc', 'Manage your account preferences and system configuration.')}</p>
      </div>

      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col md:flex-row">
        {/* Sidebar Tabs */}
        <div className={clsx("w-full md:w-64 bg-gray-50 border-gray-100 p-2 md:p-4 space-y-1", isRtl ? "border-l" : "border-r")}>
          {tabs.map((tab) => (
            <button
              key={tab.id}
        ...
        <div className="flex-1 p-6 md:p-8">
          {activeTab === 'profile' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <button className="text-sm font-medium text-red-700 hover:text-red-800 hover:underline">{t('change_avatar', 'Change Avatar')}</button>
                  <p className="text-xs text-gray-500 mt-1">{t('recommended_size', 'Recommended size: 256x256px')}</p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      defaultValue="Admin"
                      className={clsx(
                        "w-full py-2 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none",
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      disabled
                      className={clsx(
                        "w-full py-2 bg-gray-50 border border-gray-200 rounded-lg text-gray-500 cursor-not-allowed",
                        isRtl ? "pr-10 pl-4" : "pl-10 pr-4"
                      )}
                    />
                  </div>
          ...
          {activeTab === 'notifications' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">{t('email_notifications', 'Email Notifications')}</h3>
              <div className="space-y-4">
                <div className="flex items-center justify-between py-3 border-b border-gray-50">
                  <div>
                    <p className="text-sm font-medium text-gray-900">{t('new_user_registrations', 'New User Registrations')}</p>
                    <p className="text-xs text-gray-500">{t('new_user_desc', 'Receive an email when a new user signs up.')}</p>
                  </div>
                  <button
                ...
                <div className="flex items-center justify-between py-3 border-b border-gray-50">
                  <div>
                    <p className="text-sm font-medium text-gray-900">{t('urgent_case_alerts', 'Urgent Case Alerts')}</p>
                    <p className="text-xs text-gray-500">{t('urgent_case_desc', 'Get notified immediately for critical cases.')}</p>
                  </div>
                  <button
                ...
                <div className="flex items-center justify-between py-3 border-b border-gray-50">
                  <div>
                    <p className="text-sm font-medium text-gray-900">{t('weekly_reports', 'Weekly Reports')}</p>
                    <p className="text-xs text-gray-500">{t('weekly_reports_desc', 'Receive a weekly summary of donations and activity.')}</p>
                  </div>
                  <button
