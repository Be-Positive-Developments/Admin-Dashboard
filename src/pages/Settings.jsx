...
export default function SettingsPage() {
  ...
  const tabs = [
    ...
    { id: 'security', label: t('security_privacy', 'Security & Privacy'), icon: Shield },
    { id: 'system', label: t('system_preferences', 'System Preferences'), icon: Globe }
  ];

  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t('settings', 'Settings')}</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{t('settings_desc', 'Manage your account preferences and system configuration.')}</p>
      </div>
      ...
      <div className="bg-white dark:bg-[#171921] rounded-xl border border-gray-100 dark:border-[#262833] shadow-sm overflow-hidden flex flex-col md:flex-row">
        {/* Sidebar Tabs */}
        <div className={clsx("w-full md:w-64 bg-gray-50 dark:bg-[#0f1117] border-gray-100 dark:border-[#262833] p-2 md:p-4 space-y-1", isRtl ? "border-l" : "border-r")}>
          {tabs.map((tab) => (
            <button
              key={tab.id}
        ...
        <div className="flex-1 p-6 md:p-8">
          {activeTab === 'profile' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <button className="text-sm font-medium text-red-700 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 hover:underline">{t('change_avatar', 'Change Avatar')}</button>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t('recommended_size', 'Recommended size: 256x256px')}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="relative">
                    <User className={clsx("absolute top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400", isRtl ? "right-3" : "left-3")} />
                    <input
                      defaultValue="Admin"
                      className={clsx(
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('email_address', 'Email Address')}</label>
                  <div className="relative">
                    <Mail className={clsx("absolute top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400", isRtl ? "right-3" : "left-3")} />
                    <input
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="relative">
                    <Shield className={clsx("absolute top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400", isRtl ? "right-3" : "left-3")} />
                    <input
          ...
          {activeTab === 'notifications' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">{t('email_notifications', 'Email Notifications')}</h3>
              <div className="space-y-4">
                <div className="flex items-center justify-between py-3 border-b border-gray-50 dark:border-[#262833]">
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{t('new_user_registrations', 'New User Registrations')}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t('new_user_desc', 'Receive an email when a new user signs up.')}</p>
                  </div>
                  <button
                ...
                <div className="flex items-center justify-between py-3 border-b border-gray-50 dark:border-[#262833]">
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{t('urgent_case_alerts', 'Urgent Case Alerts')}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t('urgent_case_desc', 'Get notified immediately for critical cases.')}</p>
                  </div>
                  <button
                ...
                <div className="flex items-center justify-between py-3 border-b border-gray-50 dark:border-[#262833]">
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{t('weekly_reports', 'Weekly Reports')}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t('weekly_reports_desc', 'Receive a weekly summary of donations and activity.')}</p>
                  </div>
                  <button
          ...
          {(activeTab === 'security' || activeTab === 'system') && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center h-64 text-gray-400">
               ...
               <p>{t('settings_restricted', 'Settings for this section are restricted or coming soon.')}</p>
             </motion.div>
          )}

          <div className="mt-8 pt-6 border-t border-gray-100 dark:border-[#262833] flex justify-end">
            <button
              onClick={() => toast.success(t('settings_saved', 'Settings saved successfully'))}
